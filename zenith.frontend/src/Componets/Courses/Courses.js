...
export default function Courses() {
  ...
  const handleFav = async (courseID) => {
    try {
      const res = await axios.post("http://localhost:3001/auth/addFav", {
        UserID: checklogin,
        CourseID: courseID,
      });
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    >
                      ...
                      <Tooltip title="Add To My Course" onClick={() => handleFav(c?._id)}>
                        <FavoriteBorder size="md" />
                      </Tooltip>
                    </div>
                    <p className="text-gray-400">{c?.description?.substring(0, 150)}</p>
                    <div className="flex justify-between items-center mt-4">
                      <span className="text-gray-400">Likes: {c.likes}</span>
                    </div>
                  </div>
                </div>
              );
